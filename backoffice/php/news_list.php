<?php
$header_name = "News Management";
$menu_name = "News Management";
$back_link = "news_list";
...
<div class="row wrapper page-heading">
    ...
    <div class="col-xs-8 col-lg-6 text-right"><h3>&nbsp;</h3>
      <a href="home.php?show=news_add" class="btn btn-success">+ Add News</a>
    </div>
            ...
            <div class="table-responsive">
               <table class="table table-stripped table-hover table-bordered" id="deferRenderTable">
                  ...
                  <tbody>
                    <?
                    $fcat_id = 26;
                    $sql = "select * from  tt_fair_content_article a left join tt_fair_list b on a.fair_id=b.fair_id where a.fcat_id = ? order by a.fca_id DESC ";
                    $stmt = $mysqli->prepare($sql);
                    if($stmt) {
                      $stmt->bind_param('i',$fcat_id);
                      $stmt->execute();
                      $result = $stmt->get_result();
                      $numrow = $result->num_rows;
                      if($numrow>0) {
                        ...
                        while($data = $result->fetch_assoc()) {
                          ...
                          <tr>
                            ...
                            <td class="text-center vmiddle">
                              ...
                              <span><img id="e_<?=$data["fca_id"]?>" onmouseover="actionOver('e_<?=$data["fca_id"]?>','1');" onmouseout="actionOut('e_<?=$data["fca_id"]?>','1')" src="../backoffice/asset/icon_edit_null.png" height="35" onclick="window.location='home.php?show=news_edit&id=<?=$data["fca_id"]?>';" ></span>
                              &nbsp;
...
function changeStatus(id,type) {
  $.ajax({
      type: "GET",
      url: "php/news_list.php?method=chgstatus&id="+id+"&status="+type,
      dataType: "text",
...
function confirmDeleteAction(id) {
  $.ajax({
      type: "GET",
      url: "php/news_list.php?method=delete&id="+id+'&fair_id=<?=$fair_id?>&fct_id=<?=$fct_id?>',
      dataType: "text",
      success : function(data) {
        alertpopup('1','ลบรายการเรียบร้อยแล้ว');
        setTimeout(function () {top.window.location="home.php?show=news_list";},100);
      }
  });
}
</script>
