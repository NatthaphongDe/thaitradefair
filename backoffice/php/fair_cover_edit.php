<?php
$menu_name = "Edit Cover Image";
$save_link = "fair_cover_edit";
$back_link = "fair_cover";
...
if($_REQUEST["method"]=="add") {
  ...
  $sql = "update tt_fair_cover set
          cover_url = ?,
          cover_status = ?,
          cover_update_date = now(),
          cover_update_by = ?
          where cover_id = ? ";
  $stmt = $mysqli->prepare($sql);
  if($stmt) {
    $stmt->bind_param('siii',$cover_url,$cover_status,$adminid,$cover_id);
    $stmt->execute();
...
$sql = "select * from  tt_fair_cover where cover_id = ? and fair_id = ? ";
$stmt = $mysqli->prepare($sql);
if($stmt) {
  $stmt->bind_param('ii',$cover_id,$fair_id);
  $stmt->execute();
  ...
  if($numrow>0) {
    ...
    ?>
    <script type="text/javascript">
      top.window.location='home.php?show=<?=$back_link?>&id=<?=$fair_id?>';
    </script>
    <?
    exit();
  }
...
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="row">
        <div class="col-lg-12">
              ...
              <div class="row">
                <div class="col-xs-6">
                  <div class="form-group">
                    ...
                    <div class="col-lg-12">
                      <img src="<?=ROOTPATHDOMAIN?><?=$data["cover_path"]?>" width="100%">
                    </div>
                    <div class="col-lg-12">
                      <br>
                      <input type="file" name="filecover" class="form-control"  >
              ...
              <div class="row">
                <div class="col-xs-12 col-sm-6">
                  <div class="form-group">
                    ...
                    <div class="col-lg-12">
                      <input type="text" class="form-control" name="cover_url" value="<?=$data["cover_url"]?>"  />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <input type="hidden" name="cover_status" id="cover_status" value="<?=$data["cover_status"]?>">
            </form>
          </div>
        </div>
	</div>
...
function changeStatus() {
  ...
  $.ajax({
      type: "GET",
      url: "php/fair_cover_edit.php?method=chgstatus&status="+st,
      dataType: "text",
