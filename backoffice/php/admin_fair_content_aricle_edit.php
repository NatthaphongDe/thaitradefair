<?php
$save_link = "admin_fair_content_aricle_edit";
...
if($_REQUEST["method"]=="add") {
  ...
  if($stmt) {
    $stmt->bind_param('siissssssiisissssi',$fca_date,$fca_status,$adminid,$fca_title_th,$fca_title_en,$fca_detail_th,$fca_detail_en,$fc_url,$fc_youtube,$fca_pubish,$fca_tag_fair_id,$fca_tag_master_id,$fca_map_status,$fca_map_title,$fca_lat,$fca_lng,$fca_address,$fca_id);
    $stmt->execute();
  ...
  <script type="text/javascript">
    ...
    setTimeout(function () {top.window.location="../home.php?show=<?=$back_link?>&fct_id=<?=$fct_id?>";},500);
  </script>
  <?php
  exit();
...
<div class="row wrapper page-heading">
     <div class="col-xs-12">
       ...
       <ol class="breadcrumb">
           <li>
               <a href="home.php?show=<?=$back_link?>&fct_id=<?=$fct_id?>"><h3><i class="fa fa-angle-left backnav-size " aria-hidden="true"></i> <span class="backnav-size-txt">BACK</span></h3></a>
           </li>
       </ol>
            ...
            <div class="row">
                <div class="col-xs-12">
                  <button type="submit" class="btn btn-success" style="width:150px;">Save</button>
                    &nbsp;&nbsp;&nbsp;
                  <a href="home.php?show=<?=$back_link?>&fct_id=<?=$fct_id?>" class="btn btn-default" style="width:150px;">Cancel</a>
                </div>
            </div>
...
function changeStatus() {
  ...
  $.ajax({
      type: "GET",
      url: "php/admin_fair_content_aricle_edit.php?method=chgstatus&status="+st,
      dataType: "text",
...
function changePublish() {
  ...
  $.ajax({
      type: "GET",
      url: "php/admin_fair_content_aricle_edit.php?method=chgpublish&status="+st,
      dataType: "text",
      success : function(data) {
        $('._pp').empty();
        $('._pp').html(data);
...
function changeStatusMap() {
  ...
  $.ajax({
      type: "GET",
      url: "php/admin_fair_content_aricle_edit.php?method=chgstatusmap&status="+st,
      dataType: "text",
      success : function(data) {
        $('.sm_').empty();
        $('.sm_').html(data);
