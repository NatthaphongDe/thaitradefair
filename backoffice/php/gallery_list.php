<?php
$header_name = "Gallery Management";
...
<div class="row wrapper page-heading">
    ...
    <div class="col-xs-8 col-lg-6 text-right"><h3>&nbsp;</h3>
      <a href="home.php?show=gallery_add" class="btn btn-success">+ Add Gallery</a>
    </div>
          	...
          	<div class="table-responsive">
               <table class="table table-stripped table-hover table-bordered" id="deferRenderTable">
                  ...
                  <tbody>
                    <?
                    $fcat_id = 6;
                    $sql = "select * from  tt_fair_content_gallery a left join tt_fair_list b on a.fair_id=b.fair_id where a.fcat_id = ? order by a.fcg_id DESC ";
                    $stmt = $mysqli->prepare($sql);
                    if($stmt) {
                        ...
                        while($data = $result->fetch_assoc()) {
                          ...
                          <tr>
                            ...
                            <td class="text-center vmiddle">
                              ...
                              <span><img id="e_<?=$data["fcg_id"]?>" onmouseover="actionOver('e_<?=$data["fcg_id"]?>','1');" onmouseout="actionOut('e_<?=$data["fcg_id"]?>','1')" src="../backoffice/asset/icon_edit_null.png" height="35" onclick="window.location='home.php?show=gallery_edit&id=<?=$data["fcg_id"]?>';" ></span>
                              &nbsp;
...
function changeStatus(id,type) {
  $.ajax({
      type: "GET",
      url: "php/gallery_list.php?method=chgstatus&id="+id+"&status="+type,
      dataType: "text",
...
function confirmDeleteAction(id) {
  $.ajax({
      type: "GET",
      url: "php/gallery_list.php?method=delete&id="+id,
      dataType: "text",
      success : function(data) {
        alertpopup('1','ลบรายการเรียบร้อยแล้ว');
        setTimeout(function () {top.window.location="home.php?show=gallery_list";},100);
      }
  });
}
