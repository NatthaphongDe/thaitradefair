...
if ($_REQUEST["method"] == "add") {
  ...
  /* print_r($fair_item);
  exit(); */
  $sql = "select * from tt_fair_group where fair_group_abb = ? and fair_group_id != ? and fair_group_status != '9' ";
  $stmt = $mysqli->prepare($sql);
  if ($stmt) {
    $stmt->bind_param('si', $fair_group_abb, $fair_group_id);
    $stmt->execute();
...
?>
  ...
  $sql = "update tt_fair_group set fair_group_name_th = ?, fair_group_abb = ?, fair_group_link = ?, fair_group_update_date = now(), fair_group_update_by = ?, fair_group_social = ? where fair_group_id = ? ";
  $stmt = $mysqli->prepare($sql);
  if ($stmt) {
    $stmt->bind_param('sssiis', $fair_group_name_th, $fair_group_abb, $fair_group_link, $adminid, $fair_group_id, $fair_group_social);
    $stmt->execute();

    saveLogActivity(2, $_SESSION["id"], $fair_group_id, "Edit");

    $sqldel = " delete from tt_fair_group_list where fair_group_id = ?  ";
    $stmtdel = $mysqli->prepare($sqldel);
    $stmtdel->bind_param('i', $fair_group_id);
    $stmtdel->execute();
    ...
    for ($i = 0; $i < count($fair_item); $i++) {
      ...
      if ($idact > 0) {
        ...
        if ($numrowchk > 0) {
          ...
          $sqlchks = "select * from tt_fair_category order by fcat_type ASC, fcat_pos ASC ";
          $stmtchks = $mysqli->prepare($sqlchks);
          $stmtchks->execute();
          $resultchks = $stmtchks->get_result();
          $numrowchks = $resultchks->num_rows;
          if ($numrowchks > 0) {
            while ($datachks = $resultchks->fetch_assoc()) {

              $sqlc2 = " select * from tt_fair_list_cat where fair_id = ? and fcat_id = ? ";
              $stmtc2 = $mysqli->prepare($sqlc2);
              $stmtc2->bind_param('ii', $datachk["fair_id"], $datachks["fcat_id"]);
              $stmtc2->execute();
              ...
              if ($numrowc2 == 0) {
                ...
                $stmt2 = $mysqli->prepare($sql2);
                $stmt2->bind_param('iii', $datachk["fair_id"], $datachks["fcat_id"], $datachks["fcat_pos"], $datachks["fcat_cms_type"]);
                $stmt2->execute();
              }
            }
          }
    ...
    if ($_FILES['fair_group_image_path']['name'] != "") {

      $filedata = file_get_contents($_FILES["fair_group_image_path"]["tmp_name"]);

      if ($fair_group_id > 0) {
        $rmdirs = ROOTPATH . "/data/fairgroup/" . $fair_group_id . "/main";
        deleteDirectory($rmdirs);

        $rmdirs = ROOTPATH . "/data/fairgroup/" . $fair_group_id . "/resize";
        deleteDirectory($rmdirs);

        $rmdirs = ROOTPATH . "/data/fairgroup/" . $fair_group_id . "/crop";
        deleteDirectory($rmdirs);
      }
    ...
    if ($_FILES['fair_group_logo_path']['name'] != "") {

      $filedata = file_get_contents($_FILES["fair_group_logo_path"]["tmp_name"]);

      if ($fair_group_id > 0) {
        $rmdirs = ROOTPATH . "/data/fairgroup/" . $fair_group_id . "/logo";
        deleteDirectory($rmdirs);
      }
  ...
  <script type="text/javascript">
    ...
    setTimeout(function() {
      top.window.location = "../home.php?show=fair_master_list";
    }, 500);
  </script>
<?php
  exit();
...
if ($stmt) {
  ...
  if ($numrow > 0) {
    ...
    $sql2 = "select * from tt_fair_group_list a left join tt_fair_list b on a.fair_id=b.fair_id where a.fair_group_id = ? and b.fair_ditp_id > 0 and a.fair_flag = '1' and b.fair_status != '9' ";
    $stmt2 = $mysqli->prepare($sql2);
    ...
    if ($numrow2 > 0) {
      while ($data2 = $result2->fetch_assoc()) {
        array_push($array_list, $data2["fair_ditp_id"]);
      }
    }
  } else {
?>
...
<div class="row wrapper page-heading">
  <div class="col-xs-12">
    ...
    <ol class="breadcrumb">
      <li>
        <a href="home.php?show=<?= $back_link ?>">
    ...
    <div class="col-lg-12">
      <div class="ibox float-e-margins">
        <form class="form-horizontal" method="post" name="form_Suppliers_add" enctype="multipart/form-data" id="form_Suppliers_add" action="php/<?= $save_link ?>.php?method=add" target="com_m" onSubmit="pc_overlay(1);">
            ...
            <div class="row">
              <div class="col-xs-6 col-lg-6">
                <div class="form-group">
                  ...
                  <div class="col-lg-12">
                    <input type="text" name="fair_group_name_th" class="form-control" required value="<?= $data["fair_group_name_th"] ?>" />
                  </div>
                </div>
              </div>
              <div class="col-xs-6 col-lg-6">
                <div class="form-group">
                  ...
                  <div class="col-lg-12">
                    <input type="text" name="fair_group_abb" class="form-control" required value="<?= $data["fair_group_abb"] ?>" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="row">
              <div class="col-xs-6 col-lg-6">
                <div class="form-group">
                  ...
                  <div class="col-lg-12">
                    <img src="<?= ROOTPATHDOMAIN ?><?= $data["fair_group_image_path"] ?>" width="100%">
                  </div>
                  <div class="col-lg-12">
                    <br>
                    <input type="file" name="fair_group_image_path">
            ...
            <div class="row">
              <div class="col-xs-6 col-lg-6">
                <div class="form-group">
                  ...
                  <div class="col-lg-12">
                    <img src="<?= ROOTPATHDOMAIN ?><?= $data["fair_group_logo_path"] ?>" width="100%">
                  </div>
                  <div class="col-lg-12">
                    <br>
                    <input type="file" name="fair_group_logo_path">
                    <span class="text-danger " style="font-size:90%;">
            ...
            <div class="row">
              <div class="col-xs-6 col-lg-6">
                <div class="form-group">
                  ...
                  <div class="col-lg-12">
                    <input type="text" name="fair_group_link" class="form-control" value="<?= $data["fair_group_link"] ?>" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="row">
              <div class="col-xs-6 col-lg-6">
                <div class="form-group">
                  ...
                  <div class="col-lg-12">
                    <input type="text" name="fair_group_social" class="form-control" value="<?= $data["fair_group_social"] ?>" />
                  </div>
                </div>
              </div>
            </div>
                      ...
                      <div class="boxitem-inner _fairnoneselecd">
                        <? for ($i = 0; $i < count($fairlist["Acitvities"]); $i++) { ?>
                          <div class="boxitem-item _item_none">
                            <div class="boxitem-item-chk">
                            <input type="checkbox" class="act_id_ss" id="act_id_<?= $fairlist["Acitvities"][$i]["Activity_Detail"]["ID"] ?>" value="<?= $fairlist["Acitvities"][$i]["Activity_Detail"]["ID"] ?>" <? if (in_array($fairlist["Acitvities"][$i]["Activity_Detail"]["ID"], $array_list)) { ?>checked<? } ?>>
                            </div>
                            <div class="boxitem-item-name">
          ...
          <input type="hidden" name="fair_group_id" value="<?= $data["fair_group_id"] ?>">
        </form>
      </div>
    </div>
  </div>
...
<script type="text/javascript">
  ...
  function loadSelect() {
    ...
    $.ajax({
      type: "GET",
      url: "php/fair_master_edit.php?method=getdata&item=" + dataitem,
      dataType: "text",
