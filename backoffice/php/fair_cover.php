     ...
     <div class="col-xs-12">
       ...
       <ol class="breadcrumb">
           ...
           <div class="div-right text-right">
             ...
             <? } ?>
             &nbsp;&nbsp;&nbsp;
             <a href="home.php?show=fair_cover_add&id=<?=$fair_id?>" class="btn btn-success">+ Add <?=$menu_name?></a>
                          ...
                          while($data = $result->fetch_assoc()) {
                            ...
                            <tr>
                              ...
                              <td class="text-center vmiddle">
                                ...
                                <span><img id="e_<?=$data["cover_id"]?>" onmouseover="actionOver('e_<?=$data["cover_id"]?>','1');" onmouseout="actionOut('e_<?=$data["cover_id"]?>','1')" src="../backoffice/asset/icon_edit_null.png" height="35" onclick="window.location='home.php?show=fair_cover_edit&id=<?=$fair_id?>&cover_id=<?=$data["cover_id"]?>';" ></span>
                                &nbsp;
...
function changeStatus(id,type) {
  $.ajax({
      type: "GET",
      url: "php/fair_cover.php?method=chgstatus&id=<?=$fair_id?>&cover_id="+id+"&status="+type,
      dataType: "text",
...
function confirmDeleteAction(id) {
  $.ajax({
      type: "GET",
      url: "php/fair_cover.php?method=delete&id=<?=$fair_id?>&cover_id="+id,
      dataType: "text",
      success : function(data) {
        alertpopup('1','ลบรายการเรียบร้อยแล้ว');
        setTimeout(function () {top.window.location="home.php?show=fair_cover&id=<?=$fair_id?>";},500);
      }
  });
}
...
function changeTypeCoverAction(id) {
  $.ajax({
      type: "GET",
      url: "php/fair_cover.php?method=chnagetpye&type="+id+'&id=<?=$fair_id?>',
      dataType: "text",
      success : function(data) {
        setTimeout(function () {top.window.location="home.php?show=fair_cover&id=<?=$fair_id?>";},100);
      }
  });
}
</script>
