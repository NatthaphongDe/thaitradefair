...
function getExhibitorList($ditpid) {
  ...
  curl_setopt_array($curl, array(
    CURLOPT_URL => 'http://driveapi.ditp.go.th/api/getfaircatalogue',
...
$sqlf = "select * from tt_fair_list where fair_status != 9 and fair_ditp_id > 0 order by fair_id ASC limit $page,1 ";
  ...
  if($numrowf>0) {
    while($dataf = $resultf->fetch_assoc()) {
      ...
      if($json["CountItem"]>0) {
          ...
          if($json["FairCatalogue"][$i]["CompanyParent"]) {
            if($json["FairCatalogue"][$i]["CompanyParent"]["ParentID"]) {
              if($json["FairCatalogue"][$i]["CompanyParent"]["ParentID"]!="") {
                ...
                $Company_Address_Contact = $json["FairCatalogue"][$i]["ListContact"][0]["Address_Contact"];
                ...
                */
                ...
                $sql = "insert into tt_exhibitor_list (fair_id,fair_ditp_id,com_name,com_address,com_tel,com_fax,com_email,com_web,contact_name,contact_position,product_group,product_cat,product_brand,product_brand_desc,update_date,thaitrand_seller_code,thaitrand_shop_url,thaitrand_com_url) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,now(),?,?,?) ";
                $stmt = $mysqli->prepare($sql);
                if($stmt) {
                  $stmt->bind_param('iisssssssssssssss',$fairid,$fairditpid,$Company_name,$Company_Address_Contact,$Company_Telephone,$Company_Fax,$Company_Email,$Company_Website,$Company_Contanct_Full_Name,$Company_Contanct_Position,$Product_group,$Product_cat,$Product_brand,$Product_des,$thaitrand_seller_code,$thaitrand_shop_url,$thaitrand_com_url);
                  $stmt->execute();
                  ...
                  for($p=0;$p<count($json["FairCatalogue"][$i]["FairCatalogueProduct"]["ListImageUrl"]);$p++) {
                    ...
                    $sqlimg = "insert into tt_exhibitor_product (exl_id,ImageTitle,ImageUrl) values (?,?,?) ";
        ...
        echo $fairid."-".$fairditpid." ".date("Y-m-d H:i:s");
        echo "<hr>";
    }
    ...
    <script type="text/javascript">
      window.location="https://thaitrade.ibusiness.co.th/backoffice/php/get-exl.php?page=<?=$next?>"
    </script>
    <?
