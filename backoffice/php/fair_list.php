...
<div class="row wrapper page-heading">
    ...
    <div class="col-xs-8 col-lg-6 text-right"><h3>&nbsp;</h3>
      <a href="home.php?show=fair_add" class="btn btn-success">+ Create Fair Name</a>
    </div>
          	...
          	<div class="table-responsive">
               <table class="table table-stripped table-hover table-bordered" id="deferRenderTable">
                  ...
                  <tr>
                    ...
                    <th width="25%" class="text-center tr-head">Action</th>
                    <th class="text-center tr-head">Sources </th>
                  </tr>
                  </thead>
                  <tbody>
                    <?
                    $sql = "select * from tt_fair_list where fair_status != '9' order by fair_id DESC ";
                    $stmt = $mysqli->prepare($sql);
                    if($stmt) {
                      //$stmt->bind_param();
                      $stmt->execute();
                        ...
                        while($data = $result->fetch_assoc()) {
                          ...
                          <tr>
                            ...
                            <td class="text-center vmiddle">
                              ...
                              <span><img id="e_<?=$data["fair_id"]?>" onmouseover="actionOver('e_<?=$data["fair_id"]?>','1');" onmouseout="actionOut('e_<?=$data["fair_id"]?>','1')" src="../backoffice/asset/icon_edit_null.png" height="35" onclick="window.location='home.php?show=fair_edit&id=<?=$data["fair_id"]?>';" ></span>
                              &nbsp;
                              <span><img id="p_<?=$data["fair_id"]?>" onmouseover="actionOver('p_<?=$data["fair_id"]?>','4');" onmouseout="actionOut('p_<?=$data["fair_id"]?>','4')" src="../backoffice/asset/icon_img_null.png" height="35" onclick="window.location='home.php?show=fair_cover&id=<?=$data["fair_id"]?>';"></span>
                              &nbsp;
                              <span><img id="i_<?=$data["fair_id"]?>" onmouseover="actionOver('i_<?=$data["fair_id"]?>','5');" onmouseout="actionOut('i_<?=$data["fair_id"]?>','5')" src="../backoffice/asset/icon_inf_null.png" height="35" onclick="window.location='home.php?show=fair_information&id=<?=$data["fair_id"]?>';" ></span>
                              &nbsp;
                              <span><img id="c_<?=$data["fair_id"]?>" onmouseover="actionOver('c_<?=$data["fair_id"]?>','6');" onmouseout="actionOut('c_<?=$data["fair_id"]?>','6')" src="../backoffice/asset/icon_cat_null.png" height="35" onclick="window.location='home.php?show=fair_menu_list&id=<?=$data["fair_id"]?>';" ></span>
                              &nbsp;
...
function changeStatus(id,type) {
  $.ajax({
      type: "GET",
      url: "php/fair_list.php?method=chgstatus&id="+id+"&status="+type,
      dataType: "text",
...
function confirmDeleteAction(id) {
  $.ajax({
      type: "GET",
      url: "php/fair_list.php?method=delete&id="+id,
      dataType: "text",
      success : function(data) {
...
function showGetToken(id) {
  ...
  $.ajax({
      type: "GET",
      url: "php/fair_token_pass.php?id="+id,
      dataType: "text",
      success : function(data) {
        $('._modelinv').html(data);
      }
