...
if($_REQUEST["method"]=="add") {
  ...
  $sql = "insert into tt_fair_list (fair_ditp_id,fair_name,fair_year,fair_trade_start,fair_trade_end,fair_public_start,fair_public_end,fair_event_start,fair_event_end,fair_vanue,fair_url,fair_status,fair_create_date,fair_create_by,fair_update_date,fair_update_by,fair_map_status,fair_map_title,fair_lat,fair_lng,fait_map_address) values ('0',?,?,?,?,?,?,?,?,?,?,?,now(),?,now(),?,?,?,?,?,?) ";
  $stmt = $mysqli->prepare($sql);
  if($stmt) {
    $stmt->bind_param('sissssssssiiiissss',$fair_name,$fair_year,$fair_trade_start,$fair_trade_end,$fair_public_start,$fair_public_end,$fair_event_start,$fair_event_end,$fair_vanue,$fair_url,$fair_status,$adminid,$adminid,$fair_map_status,$fair_map_title,$fair_lat,$fair_lng,$fait_map_address);
    $stmt->execute();
...
function changeStatus() {
  ...
  $.ajax({
      type: "GET",
      url: "php/adfair_add.php?method=chgstatus&status="+st,
      dataType: "text",
...
function changeStatusMap() {
  ...
  $.ajax({
      type: "GET",
      url: "php/adfair_add.php?method=chgstatusmap&status="+st,
      dataType: "text",
      success : function(data) {
        $('.sm_').empty();
        $('.sm_').html(data);
