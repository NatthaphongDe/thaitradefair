...
if($_REQUEST["method"]=="add") {
  ...
  $adminid = $_SESSION["id"];
  $fair_grop_id = $_POST["fair_grop_id"];
  $admin_ids = $_POST["admin_id"];

  $sql2 = "delete from tt_fair_group_admin where fair_group_id = ? ";
  $stmt2 = $mysqli->prepare($sql2);
  $stmt2->bind_param('i',$fair_grop_id);
  $stmt2->execute();
  ...
  foreach ($admin_ids as $key => $value) {
    $id = (int)$value;
    if($id>0) {
      $sqlup = "insert into tt_fair_group_admin (fair_group_id,admin_id) values (?,?) ";
      $stmtup = $mysqli->prepare($sqlup);
      $stmtup->bind_param('ii',$fair_grop_id,$id);
      $stmtup->execute();
    }
  }
  ?>
...
?>
<form class="form-horizontal" method="post" name="form_Suppliers_add" enctype="multipart/form-data" id="form_Suppliers_add" action="php/fair_master_admin.php?method=add" target="com_m" onSubmit="pc_overlay(1);" >
    ...
    <div class="modal-body">
      ...
      <div class="table-responsive">
         <table class="table table-stripped table-hover table-bordered" id="deferRenderTableAdmin">
            ...
            <tbody>
              <?
              $sql = "select * from tt_admin a left join tt_admin_role b on a.user_type=b.role_id where a.enable != '9' and b.role_lv = '2' and b.role_status != '9' order by a.username ASC ";
                  ...
                  while($data = $result->fetch_assoc()) {
                    ...
                    <tr>
                      <td class="text-center vmiddle">
                        <input type="checkbox" name="admin_id[<?=$runno?>]" class="act_id_ss" value="<?=$data["id"]?>" <? if(checkAdminFairGroup($fair_grop_id,$data["id"])) { echo "checked"; } ?>>
                      </td>
      ...
      <div class="row">
          <div class="col-xs-12">
            <button type="submit" class="btn btn-success" style="width:150px;">Save</button>
              &nbsp;&nbsp;&nbsp;
            <a href="home.php?show=fair_master_list" class="btn btn-default" style="width:150px;">Cancel</a>
          </div>
      </div>
...
<input type="hidden" name="fair_grop_id" value="<?=$fair_grop_id?>">
</form>
