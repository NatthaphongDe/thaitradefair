...
$list[] = ['mail' => $dataeld['sso_email'], 'name' => $dataeld['sso_name_en']];
...
$sqlelc = " SELECT * FROM `tt_meeting_list` WHERE `meeting_list_mid` = $meeting_id AND meeting_list_status = 1  ORDER BY `tt_meeting_list`.`meeting_list_id` ASC ";
...
if ($numrowelc > 0) {
  while ($dataelc = $resultelc->fetch_assoc()) {
    ...
    // print_r($dataelc);
    // echo "</pre>";
    $meeting_list_room = $dataelc['meeting_list_room'];
    ...
    $bx_text .= '<div style="padding: 10px 15px;border-radius: 10px;border: solid 1px #b2b2b2;background-color: #fff;font-weight: bold;margin-bottom: 10px;">
                    ...
                    <div style="text-align: center;margin-top: 10px;margin-bottom: 5px;">
                      <a style=" width: 222px; padding: 7px 21px 6px 22px; border-radius: 8px;  background-color: #378dd7;color: #fff; text-decoration: inherit;" href="https://conference01.ibusiness.co.th/' . $meeting_list_room . '">GO TO MEETING ROOM</a>
                    </div>
                  </div>';
  }
}
                ...
                <div class="b4" style="background-image: url(<?php echo ROOTPATHDOMAIN; ?>assets/images/mail-b-3.png);background-repeat: no-repeat;background-position: right bottom;background-size: auto 400px;">
                  <div class="" style="padding:40px 50px 15px 50px;text-align: left">
                    ...
                    <div class="" style="text-align: left; margin-top: 35px;color:#4a4a4a;">
                      Hi <b style="color:#378dd7"><?php echo $value['name']; ?></b>
...
foreach ($list as $key => $value) {
  ...
  $mailx = explode(',', $value['mail']);
  foreach ($mailx as $key2 => $value2) {
    $body = [
      'Messages' => [
        [
          'From' => [
    ...
    /* $body = [
      ...
      "to" => [
        [
          "email" => trim($value2),    // ตรวจสอบว่า $value คือ email string
          "name"  => $To_Name   // ชื่อผู้รับ
        ]
      ],
      "subject"     => "Thaitrade Online Meeting",
      "htmlContent" => $value['message']  // เปลี่ยนจาก HTMLPart เป็น htmlContent
    ];
