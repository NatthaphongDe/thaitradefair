...
$top_menu_active = "official-hotels";
$yearnow = date("Y");
  ...
  if($numrowfair>0) {
    ...
    <div class="row fair-calendar-list mb-3">
        ...
        <div class="col-12 col-sm-6 p-3">
            <a href="<?=ROOTPATHDOMAIN?>fair/<?=$datafair["fair_id"]?>/<?=urlencode($datafair["fair_name"])?>/"><h3 class="title"><?=$datafair["fair_name"]?></h3></a>
            <div class="d-block p">
                <img src="<?=ROOTPATHDOMAIN?>assets/images/anticon-calendar.png" class="icon-p" /><b>Public:</b>
                <?=getMonthEng(date("m",strtotime($datafair["fair_public_end"])))?>
                 <?=date("d",strtotime($datafair["fair_public_start"]))?>-<?=date("d",strtotime($datafair["fair_public_end"]))?>, <?=date("Y",strtotime($datafair["fair_public_end"]))?>
            </div>
            <div class="d-block p">
                <img src="<?=ROOTPATHDOMAIN?>assets/images/anticon-calendar.png" class="icon-p" /><b>Trade:</b>
                <?=getMonthEng(date("m",strtotime($datafair["fair_trade_end"])))?>
                 <?=date("d",strtotime($datafair["fair_trade_start"]))?>-<?=date("d",strtotime($datafair["fair_trade_end"]))?>, <?=date("Y",strtotime($datafair["fair_trade_end"]))?>
            </div>
            <div class="d-block p">
                <img src="<?=ROOTPATHDOMAIN?>assets/images/anticon-pin-drop-material.png"
                    class="icon-p" /><b>Venue:</b>
                    <?=$datafair["fair_vanue"]?>
            </div>
...
<head>
  <title>Thailand Trade Fair  | Thailand Exhibition Calendar <?=date("Y")?> - Bangkok Show <?=date("Y")?> - Thai Trade Fair <?=date("Y")?> : Official Hotels</title>
  <meta name="description" content="Thailand Trade Fair  | Thailand Exhibition Calendar <?=date("Y")?> - Bangkok Show <?=date("Y")?> - Thai Trade Fair <?=date("Y")?> : Official Hotels">
...
<body class="d-flex flex-column h-100">
                        ...
                        <div class="col px-0">
                          <div id="fair-calendar-title" class="row _homebanner_bgheight">
                              <div class="col-12 align-self-center">
                                  <h1 class="title">OFFICIAL HOTELS</h1>
                              </div>
                          </div>
                        </div>
                    </div>
                ...
                <div class="container ">
                    ...
                    <div class="row my-3 px-0 px-md-5 mb-5">
                        <div class="col px-4 px-md-0 _datashow">
                          <?
                          $sqlfair = " select * from tt_fair_list a left join tt_fair_group_list b on a.fair_id=b.fair_id  left join tt_fair_group c on b.fair_group_id=c.fair_group_id where a.fair_status = '1' and a.fair_year = ? and b.fair_flag = '1' and c.fair_group_status = '1' order by fair_event_start DESC ";
                          $stmtfair = $mysqli->prepare($sqlfair);
    ...
    <script type="text/javascript">
      function loadData() {
        ...
        $.ajax({
            type: "GET",
            url: "<?=ROOTPATHDOMAIN?>/official-hotels.php?method=load&m="+m+"&y="+y+"&o="+o,
            dataType: "text",
