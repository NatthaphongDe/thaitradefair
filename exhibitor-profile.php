...
$top_menu_active = "exhibitor-profile";

$exp_id = '';
$exl_id = (int)$_GET["exl_id"];

$sqleld = " select * from tt_exhibitor_list where exl_id = ? ";
$stmteld = $mysqli->prepare($sqleld);
$stmteld->bind_param('i',$exl_id);
$stmteld->execute();
...
<head>
  <title>Thailand Trade Fair  | Thailand Exhibition Calendar <?=date("Y")?> - Bangkok Show <?=date("Y")?> - Thai Trade Fair <?=date("Y")?> (<?=$dataeld["com_name"]?>)</title>
